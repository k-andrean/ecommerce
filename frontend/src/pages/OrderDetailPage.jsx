...
import { useGetOrderDetailQuery } from "services/ordersAPI";
...
const OrderDetailPage = () => {
  ...
  const { data: orderDetailsRespData, isSuccess: isSuccessOrderDetails } =
    useGetOrderDetailQuery(currentUserId, orderId);
  ...
  useEffect(() => {
    if (isSuccessOrderDetails && orderDetailsRespData) {
      ...
      const fetchUserData = async () => {
        try {
          const userDataResp = await fetchAxios.get(
            `/users/${orderDetailsRespData.user_id}`
          );
      ...
      <div className="mx-auto max-w-3xl">
        <div className="max-w-xl">
          ...
          <p className="mt-2 text-base text-gray-500">
            Your order #{orderDetailsData.id} has shipped and will be with you
            soon.
          </p>
          ...
          <div className="sm:ml-40 sm:pl-6">
                ...
                <dd className="mt-2 text-gray-700">
                  <address className="not-italic">
                    ...
                    <span className="block">
                      {orderDetailsData?.shipping_info?.address}
                    </span>
                    <span className="block">
                      {orderDetailsData?.shipping_info?.city}
                    </span>
                  </address>
                </dd>
              </div>
            ...
            <dl className="grid grid-cols-2 gap-x-6 border-t border-gray-200 py-10 text-sm">
              ...
              <div>
                ...
                <dd className="mt-2 text-gray-700">
                  <p>
                    {orderDetailsData?.shipping_info?.shipping_option?.title}
                  </p>
                  <p>
                    Takes up to{" "}
                    {
                      orderDetailsData?.shipping_info?.shipping_option
                        ?.turnaround
                    }{" "}
                    working days
                  </p>
            ...
            <dl className="space-y-6 border-t border-gray-200 pt-10 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="text-lg font-bold text-gray-900">
                  Rp {Math.floor(orderDetailsData?.total_amount)}
                </dd>
              </div>
            </dl>
          </div>
